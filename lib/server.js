...
function serve (builder, options) {
  ...
  if (options.ssl) {
    ...
    var keyPath = options.ssl.key || path.join(process.cwd(), 'ssl/server.key')
    var certPath = options.ssl.cert || path.join(process.cwd(), 'ssl/server.crt')
    var sslOptions
  ...
  // We don't want the server to run for certain unit tests
  if (options.serverListen !== false) {
